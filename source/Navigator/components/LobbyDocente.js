function LobbyDocente({
  ...
  React.useEffect(() => {
    ...
    es.onmessage = e => {
      ...
      if (data.tipo === 'stato-iniziale') {
        ...
        showAlert(`${data.nome} ha lasciato la visita.`, {
          type: 'info'
        });
  ...
  async function handleAvvia() {
    setAvviando(true);
    try {
      await fetch(`/api/sessioni/${encodeURIComponent(codice)}/avvia`, {
        method: 'POST'
      });
    } finally {
      setAvviando(false);
  ...
  async function handleFermaAudio() {
    setAudioAvviato(false);
    try {
      await fetch(`/api/sessioni/${encodeURIComponent(codice)}/audio/stop`, {
        method: 'POST'
      });
    } catch (_) {}
  }
  async function handleAvviaQuiz() {
    ...
    try {
      const r = await fetch(`/api/sessioni/${encodeURIComponent(codice)}/quiz/avvia`, {
        method: 'POST'
      });
      const d = await r.json();
  ...
  async function handleTerminaQuizOra() {
    ...
    try {
      await fetch(`/api/sessioni/${encodeURIComponent(codice)}/quiz/termina`, {
        method: 'POST'
      });
    } finally {
  ...
  const museoHeader = museo && /*#__PURE__*/React.createElement("div", {
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, museoHeader, backBar, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Lobby di Attesa · Docente"), /*#__PURE__*/React.createElement("h1", {
    className: "lobby-title"
  }, visitaNome), /*#__PURE__*/React.createElement("div", {
    className: "lobby-code-box"
  }, /*#__PURE__*/React.createElement("span", {
    className: "lobby-code-label"
  }, "Codice da condividere con i partecipanti:"), /*#__PURE__*/React.createElement("span", {
    className: "lobby-code"
  }, codice))), /*#__PURE__*/React.createElement("section", {
    className: "lobby-panel"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "lobby-panel-title"
  }, "Partecipanti connessi", /*#__PURE__*/React.createElement("span", {
    className: "lobby-count-badge"
  }, studenti.length)), studenti.length === 0 ? /*#__PURE__*/React.createElement("p", {
    className: "lobby-empty"
  }, "In attesa che i partecipanti si connettano…") : /*#__PURE__*/React.createElement("ul", {
    className: "lobby-students-list"
  }, studenti.map((s, i) => {
    const roleCfg = ROLE_MAP[s.ruolo];
    return /*#__PURE__*/React.createElement("li", {
      key: i,
      className: "lobby-student-item"
    }, /*#__PURE__*/React.createElement("span", {
      className: "lobby-avatar",
      style: roleCfg ? avatarStyle(roleCfg) : undefined
    }, roleCfg ? '' : s.nome[0]), s.nome);
  }))), /*#__PURE__*/React.createElement("button", {
    className: "inizia-btn",
    disabled: avviando,
    onClick: handleAvvia
  }, avviando ? 'Avvio in corso…' : studenti.length === 0 ? 'Inizia Visita da solo/a' : `Inizia Visita con ${studenti.length} partecipant${studenti.length === 1 ? 'e' : 'i'}`))));
}