function VisitaItemScreen({
  ...
  const AMENITY_LABELS = {
    ...
    caffetteria: 'La caffetteria',
    ingresso: "L'ingresso",
    info_point: 'Il punto informazioni',
    guardaroba: 'Il guardaroba',
    audio_guida: "L'audioguida",
    auditorium: "L'auditorium",
    conferenze: 'La sala conferenze',
    educazione: "L'area educativa",
    scale_mobili: 'Le scale mobili',
    bagno_disabili: 'Il bagno per disabili',
    bagno_donne: 'Il bagno delle donne',
    bagno_uomini: "Il bagno degli uomini",
    allattamento: 'La sala allattamento',
    area_riposo: "L'area di riposo",
    negozio: 'Il negozio',
  ...
  async function locateAmenity(kind) {
    ...
    try {
      const [rMuseo, rOpere] = await Promise.all([fetch(`/api/musei/${encodeURIComponent(activeItem.museumId)}`), fetch(`/api/opere?codiceIsil=${encodeURIComponent(activeItem.museumId)}`)]);
      ...
      if (!piani.length) {
        speakAssistant(`${label}: mappa non disponibile per questo museo.`);
        return;
      }
      ...
      for (let i = 0; i < piani.length; i++) {
        ...
        try {
          ...
          if (matchedFeat) {
            foundFloors.push(i);
            matchedIdByFloor[i] = matchedFeat.properties.room_id;
          }
        } catch (_) {}
      }
      ...
      const nearestIdx = foundFloors.reduce((best, i) => Math.abs(i - refIdx) < Math.abs(best - refIdx) ? i : best, foundFloors[0]);
      ...
      setLogisticsTarget({
        roomId: matchedIdByFloor[nearestIdx] || kind,
        piano: piani[nearestIdx]?.piano
      });
    } catch (_) {
  ...
  async function locateOpera(query) {
    ...
    try {
      ...
      const found = opere.find(o => normalizeVoiceText(o.operaId) === nq) || opere.find(o => {
      ...
      for (let i = 0; i < piani.length; i++) {
        ...
        try {
          ...
          const geo = await res.json();
          const feats = geo.features || [];
          if (feats.some(f => f.properties?.room_id === sala)) {
            floorIdx = i;
            break;
          }
        } catch (_) {}
  ...
  React.useEffect(() => {
    ...
    Promise.allSettled(allIds.map(id => fetch(`/api/items/${encodeURIComponent(id)}`).then(r => r.json()).then(d => d.data))).then(results => {
      if (cancelled) return;
      results.forEach(res => {
  ...
  React.useEffect(() => {
    ...
    setLoading(true);
    let cancelled = false;
    Promise.all(ids.map(id => fetch(`/api/items/${encodeURIComponent(id)}`).then(r => r.json()).then(d => d.data || null).catch(() => null))).then(items => {
      if (cancelled) return;
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    fetch(`/api/opere?codiceIsil=${encodeURIComponent(activeItem.museumId)}`).then(r => r.json()).then(d => {
      if (cancelled) return;
  ...
  React.useEffect(() => {
    audioAvviatoRef.current = audioAvviato;
  }, [audioAvviato]);
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    ...
    fetch('/api/tts', {
      ...
      if (audioAvviatoRef.current && !ttsMutedRef.current) {
        audio.play().then(() => {
          if (!isDocente && nomeAssegnato && codice) reportAscolto(true);
        }).catch(() => {});
      }
    }).catch(() => {}).finally(() => {
  ...
  function handleAudioButtonClick() {
    ...
    if (inAscolto) {
      mostraPopupAscolto('Non puoi avviare un nuovo audio');
      return;
    }
  ...
  async function eseguiTermina() {
    setTerminating(true);
    try {
      await fetch(`/api/sessioni/${encodeURIComponent(codice)}/chiudi`, {
        method: 'POST'
      });
    } finally {
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "visita-item-root"
  }, /*#__PURE__*/React.createElement("div", {
    className: "visita-item-header"
    ...
    className: "visita-chat-badge"
  }, unread > 9 ? '9+' : unread)), !isDocente && /*#__PURE__*/React.createElement("button", {
    className: "visita-item-exit-btn",
    onClick: onBack
  }, "← Esci"))), /*#__PURE__*/React.createElement("div", {
    className: "visita-item-progress"
  }, /*#__PURE__*/React.createElement("div", {
    className: "visita-item-progress-fill",
    style: {
      width: `${progress}%`
    }
  })), /*#__PURE__*/React.createElement("div", {
    className: "visita-item-body"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "visita-item-loading"
  }, /*#__PURE__*/React.createElement("div", {
    className: "nav-spinner"
  }), /*#__PURE__*/React.createElement("p", null, "Caricamento item…")), !loading && !operaGroup && /*#__PURE__*/React.createElement("p", {
    className: "visita-item-empty"
  }, "La visita non contiene opere."), !loading && operaGroup && !activeItem && /*#__PURE__*/React.createElement("p", {
    className: "visita-item-empty"
  }, "Nessun item disponibile per questa opera."), !loading && activeItem && /*#__PURE__*/React.createElement("div", {
  ...
  }), activePanel === 'opera' && !activeItem.image && !operaFallbackImg && /*#__PURE__*/React.createElement("p", {
    className: "visita-item-empty"
  }, "Nessuna immagine disponibile per questa opera."), /*#__PURE__*/React.createElement("div", {
    className: "visita-toni-bar"
  }, TONI_CONFIG.map(t => /*#__PURE__*/React.createElement("button", {
    key: t.key,
    className: `visita-tono-btn${tono === t.key ? ' visita-tono-btn--active' : ''}`,
    onClick: () => setTono(t.key)
  }, t.label))), /*#__PURE__*/React.createElement("div", {
    className: "visita-toni-bar visita-durate-bar"
  }, DURATE_CONFIG.map(d => /*#__PURE__*/React.createElement("button", {
    key: d.key,
    className: `visita-tono-btn${durata === d.key ? ' visita-tono-btn--active' : ''}`,
    onClick: () => setDurata(d.key)
  }, d.label))), micOn && /*#__PURE__*/React.createElement("p", {
    className: "visita-voice-transcript"
  }, /*#__PURE__*/React.createElement("i", {
    className: "fa-solid fa-microphone me-1"
  }), voiceTranscript || 'In ascolto…'), /*#__PURE__*/React.createElement("p", {
    className: "visita-item-text"
  }, toneText(activeItem.toni?.[tono], durata) || /*#__PURE__*/React.createElement("em", null, "Nessun contenuto per questo tono e durata."))))), isDocente && !quiz && /*#__PURE__*/React.createElement("div", {
    className: "visita-audio-bar"
  }, /*#__PURE__*/React.createElement("button", {
    type: "button",
    className: `visita-audio-btn${audioAvviato ? ' visita-audio-btn--avviato' : ''}`,
    onClick: handleAudioButtonClick
  }, /*#__PURE__*/React.createElement("i", {
    className: `fa-solid ${audioAvviato ? 'fa-stop' : 'fa-volume-high'}`
  }), audioAvviato ? 'Ferma audio' : 'Avvia audio per tutti')), isDocente && !quiz && /*#__PURE__*/React.createElement("div", {
    className: "visita-item-nav"
  }, /*#__PURE__*/React.createElement("button", {
    ...
    className: "visita-nav-counter"
  }, currentIdx + 1, " / ", totalItems), /*#__PURE__*/React.createElement("button", {
    className: "visita-nav-btn visita-nav-btn--next",
    onClick: () => navigate('avanti'),
    disabled: currentIdx >= totalItems - 1 || navigating
  }, "Prossimo →")), isDocente && /*#__PURE__*/React.createElement("div", {
    className: "visita-termina-bar"
  }, /*#__PURE__*/React.createElement("button", {
    ...
    className: "quiz-prompt-modal"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "quiz-prompt-title"
  }, "Avviare il quiz finale?"), /*#__PURE__*/React.createElement("p", {
    className: "quiz-prompt-text"
  }, "Questa visita ha un quiz associato. Puoi farlo partire adesso (i partecipanti risponderanno dai loro dispositivi) oppure terminare la visita senza quiz."), /*#__PURE__*/React.createElement("div", {
    className: "quiz-prompt-actions"
  }, /*#__PURE__*/React.createElement("button", {
    className: "quiz-prompt-btn quiz-prompt-btn--ghost",
    onClick: () => setQuizPromptOpen(false)
  }, "Annulla"), /*#__PURE__*/React.createElement("button", {
    className: "quiz-prompt-btn quiz-prompt-btn--danger",
    onClick: handleTerminaSenzaQuiz
  }, "Termina senza quiz"), /*#__PURE__*/React.createElement("button", {
    className: "quiz-prompt-btn quiz-prompt-btn--primary",
    onClick: handleAvviaQuizDaPrompt,
    disabled: quizAvviando
  }, quizAvviando ? 'Avvio in corso…' : 'Avvia Quiz'))), isDocente && ascoltoPopupOpen && /*#__PURE__*/React.createElement("div", {
    className: "ui-modal-backdrop show",
    onClick: () => setAscoltoPopupOpen(false)
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "ui-modal-icon warning ui-modal-icon--hourglass"
  }, /*#__PURE__*/React.createElement("i", {
    className: `fa-solid ${ascoltoCountdown > 40 ? 'fa-hourglass-start' : ascoltoCountdown > 20 ? 'fa-hourglass-half' : 'fa-hourglass-end'}`
  })), /*#__PURE__*/React.createElement("div", {
    className: "ui-modal-title"
  }, ascoltoPopupTitolo), /*#__PURE__*/React.createElement("div", {
    className: "ui-modal-msg"
  }, "Ci sono ancora studenti in ascolto!", /*#__PURE__*/React.createElement("br", null), "Attendi qualche secondo e riprova.", /*#__PURE__*/React.createElement("br", null), /*#__PURE__*/React.createElement("strong", {
    style: {
      fontSize: '1.3rem'
    }
  }, ascoltoCountdown, "s")), /*#__PURE__*/React.createElement("div", {
    className: "ui-modal-actions"
  }, /*#__PURE__*/React.createElement("button", {
    type: "button",
    className: "ui-modal-btn ui-modal-btn-primary",
    onClick: () => setAscoltoPopupOpen(false)
  }, "Ho capito")))), isDocente && itemsMenuOpen && /*#__PURE__*/React.createElement("div", {
    className: "visita-chat-overlay",
    onClick: () => setItemsMenuOpen(false)
  }), isDocente && /*#__PURE__*/React.createElement("div", {
    className: `visita-chat-panel${itemsMenuOpen ? ' visita-chat-panel--open' : ''}`
  }, /*#__PURE__*/React.createElement("div", {
    className: "visita-chat-panel-header"
  }, /*#__PURE__*/React.createElement("span", {
    className: "visita-chat-panel-title"
  }, /*#__PURE__*/React.createElement("i", {
    className: "fa-solid fa-list",
    style: {
      marginRight: '8px'
    }
  }), "Vai a un'opera"), /*#__PURE__*/React.createElement("button", {
    className: "visita-chat-close",
    onClick: () => setItemsMenuOpen(false)
  }, "✕")), /*#__PURE__*/React.createElement("div", {
    className: "visita-chat-msgs"
  }, visitaItems.length === 0 ? /*#__PURE__*/React.createElement("p", {
    className: "visita-chat-empty"
  }, "Caricamento elenco opere…") : visitaItems.map((it, idx) => /*#__PURE__*/React.createElement("button", {
    key: it.operaId || idx,
    className: `visita-jump-item${idx === currentIdx ? ' visita-jump-item--active' : ''}`,
    onClick: () => jumpToItem(idx),
    disabled: navigating
  }, /*#__PURE__*/React.createElement("span", {
    className: "visita-jump-item-num"
  }, idx + 1), /*#__PURE__*/React.createElement("span", {
    className: "visita-jump-item-name"
  }, it.operaId), idx === currentIdx && /*#__PURE__*/React.createElement("i", {
    className: "fa-solid fa-volume-high"
  }))))), isDocente && monitorOpen && /*#__PURE__*/React.createElement("div", {
    className: "visita-chat-overlay",
    onClick: () => setMonitorOpen(false)
  }), isDocente && /*#__PURE__*/React.createElement("div", {
    className: `visita-chat-panel${monitorOpen ? ' visita-chat-panel--open' : ''}`
  }, /*#__PURE__*/React.createElement("div", {
    className: "visita-chat-panel-header"
  }, /*#__PURE__*/React.createElement("span", {
    className: "visita-chat-panel-title"
  ...
  }), "Monitoraggio partecipanti"), /*#__PURE__*/React.createElement("button", {
    className: "visita-chat-close",
    onClick: () => setMonitorOpen(false)
  }, "✕")), /*#__PURE__*/React.createElement("div", {
    className: "visita-chat-msgs"
  }, Object.keys(studentTono).length === 0 ? /*#__PURE__*/React.createElement("p", {
    className: "visita-chat-empty"
  }, "Nessuna richiesta ancora.", /*#__PURE__*/React.createElement("br", null), "Qui vedrai in tempo reale i cambi di tono/linguaggio di ogni partecipante.") : Object.entries(studentTono).sort((a, b) => b[1].timestamp - a[1].timestamp).map(([nome, info]) => {
    ...
    }, new Date(info.timestamp).toLocaleTimeString('it-IT', {
      hour: '2-digit',
      minute: '2-digit'
    }))), /*#__PURE__*/React.createElement("p", {
  ...
  }), "Messaggi", messages.length > 0 && /*#__PURE__*/React.createElement("span", {
    className: "visita-chat-count"
  }, messages.length)), /*#__PURE__*/React.createElement("button", {
    className: "visita-chat-close",
    onClick: () => setChatOpen(false)
  }, "✕")), /*#__PURE__*/React.createElement("div", {
    className: "visita-chat-msgs"
